...
The API does not allow you to request page numbers above (equal to?) 200.
            1.2.0.2 PROBLEMSTRS
In the globals.ts file I have an array called problemStrs. These are strings which show up in tags sometimes and cause problems for the API. (it's an error on rule34.xxx's fault basically.) They are called HTML Entities I believe. It's got to do with certain characters not being allowed in a url I think? And so these characters are represented in url-friendly code. For instance "&eacute;" instead of the e with an accent over it symbol.
The way Rule34 Tools handles this is it checks tags for these problem strings and then ignores the tag if it contains one. It's a bandaid fix, but it works pretty alright. I have found a full list of these symbols: https://html.spec.whatwg.org/multipage/named-characters.html#named-character-references but the list is really long so I'm dubious of adding them all into problemStrs. A better fix for this is very possible.
...
Gosh... I'm running out of steam. I might break format here and just word vomit.
        1.3.1 WORD VOMIT
Rule34 Advanced Search is where I use Rule34 Tools to rate posts as well as any other cool application I want. It is a website that is run through github pages for free, and it can be accessed from anywhere with an internet connection.
...
And also I have an idea for a tag rater that does sort of a combination of l1 and l2 tag rating kinda simply. It would bring up the search from the prompt it's rating for and take a census of the tags in that search. And then it would start going through each of the tags in its census and pulling up searches of those tags. And maybe every time a tag shows up in the initial first search of the prompt you're looking for it gets a +1, and every time it shows up in a census of a tag that showed up in the census of the prompt you're rating for it gets a +0.1. And then you can just keep that going for as long as you want to keep gathering data, and then once you're satisfied you stop it and have it rate tags for you, which should be instant if it's loaded all the rating information into its memory beforehand.
        1.3.2 TYPESCRIPT COMPILER
The typescript compiler interfacing with github pages has been perhaps the hardest part of this entire project for me. The idea is that typescript is much nicer to write in than javascript, but websites don't actually run typescript, they run javascript. So you use the typescript compiler to rewrite your typescript as javascript, and then that javascript is what you give to your website to run.
The problem is that apparently there's a bajillion different types of javascript, and github pages is okay with some types of javascript but not okay with others. And so you go into tsconfig.json and flip variables around blindly for days to change the type of javascript that's being generated. A nightmare.
The settings I have in the tsconfig now are working, but I do not understand why. Also there's a problem with it where vscode/typescript will tell me that I can import files without including the .js extension, but if I do that the website throws an error. So I have to manually check that all my extensions are there, since they are not always added by default and the type checker won't flag it as an error. Annoying.
For future projects I would for sure redo the entire tsconfig stuff and try to get something that works better. I'm told that there may be templates out there which automatically set up the tsconfig for you. I'm told that's worth looking into.
