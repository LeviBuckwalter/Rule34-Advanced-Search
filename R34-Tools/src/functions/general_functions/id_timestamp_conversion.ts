type id = number
type ts = number
type tsIdPair = {timestamp: ts, id: id}

const dataPoints: tsIdPair[] = [
...
const tsToIdMap: Map<ts, id> = new Map()
for (const dataPoint of dataPoints) {
    tsToIdMap.set(dataPoint.timestamp, dataPoint.id)
}
export function tsToId(givenTs: ts): id {
    let recenter: tsIdPair | null = null
    let older: tsIdPair | null = null
    for (const rawEntry of tsToIdMap.entries()) {
        const entry: tsIdPair = {timestamp: rawEntry[0], id: rawEntry[1]}
    ...
    if (recenter !== null)/*if givenTs is between two known datapoints*/{
        ...
        let olderer: tsIdPair | undefined = undefined
        for (const rawEntry of tsToIdMap.entries()) {
            const entry: tsIdPair = {timestamp: rawEntry[0], id: rawEntry[1]}
...
const idToTsMap: Map<number, number> = new Map()
for (const dataPoint of dataPoints) {
    idToTsMap.set(dataPoint.id, dataPoint.timestamp)
}
export function idToTs(givenId: id): ts {
    let recenter: tsIdPair | null = null
    let older: tsIdPair | null = null
    for (const rawEntry of idToTsMap.entries()) {
        const entry: tsIdPair = {timestamp: rawEntry[1], id: rawEntry[0]}
...
export function dateToId(date: Date): number {
    const milliseconds = date.getTime()
    const seconds = milliseconds/1000
    return tsToId(seconds)
}

export function idToDate(id: number): Date {
    const seconds = idToTs(id)
    const milliseconds = seconds*1000
    return new Date(milliseconds)
}

export function easyDate(month: number, day: number, year: number): Date {
    if (year < 100) {
        year += 2000
    }
