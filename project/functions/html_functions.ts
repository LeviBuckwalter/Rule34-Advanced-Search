...
export function instantiateElements<T extends { [id: string]: new () => HTMLElement }>(
    givenObj: T
    ...
    */
    ...
    const retObj = {} as { [K in keyof T]: InstanceType<T[K]> }

    for (const id of Object.keys(givenObj)) {
        retObj[id as keyof T] = smartGetElement(id, givenObj[id as keyof T]) as InstanceType<T[keyof T]>
    }
    return retObj
}
