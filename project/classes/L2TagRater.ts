...
export class l2TagRater {
    ...
    async init() {
        this.comTtrb = await getCommonness(this.ttrb)
        this.censusTtrb = new Census(await getPosts(this.ttrb, this.parameters.ttrbCensusSize, { lookInCache: false, storeInCache: false }))
    ...
    async rate(ttr: string): Promise<number> {
        ...
        const l1TagComs: { [tag: string]: Promise<number> } = {}//level 1 tag commonnesses (p(t1x))
        ...
        for (const l1Tag of chosenL1Tags) {
            ...
            const comT1x = await l1TagComs[l1Tag]
