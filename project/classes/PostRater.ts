...
export class PostRater {
    parameters: { lvl2RateMaxPosts: number }
    generalSample: SortedSample
    rateForCensus: Census
    rateByTag: string
    tagRatingCacheLvl1: Cache<number | null>
    tagRatingCacheLvl2: Cache<number | null>

    constructor(generalSample: SortedSample, rateForCensus: Census, rateForTag: string) {
    ...
    private rateTagLvl1(t: string): number {
        /*
    ...
    ratePost(p: Post, level: number): number | null {
        ...
        if (ratings.length === 0) { return Math.log(0.5) }

        return this.combineProbabilities(...ratings)
    }

    combineProbabilities(...probLogsFor: number[]): number {
